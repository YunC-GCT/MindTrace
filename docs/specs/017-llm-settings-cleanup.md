...
- **L1**(per-vendor customVendors 持久化):spec 016 §8 已知限制,**需扩 LlmConfig 加 `KEY_CUSTOM_VENDORS` JSON 数组**,工作量大(2-3 hr),独立 spec(后续 018)
- **L2**(per-vendor models 持久化):同上,需扩 LlmConfig + VendorPicker
- **L3**(per-vendor API Key):需扩 AssetStoreKit alias
