...
test('run coordinator and snapshot decoder expose the required seams', () => {
  ...
  assert.match(service, /private readonly coordinator: ConversationRunCoordinator/);
  assert.match(service, /const request: ConversationRunRequest = \{ kind: kind \}/);
  assert.match(service, /return this\.coordinator\.start\(sessionId, request\)/);
  assert.match(service, /executeAcceptedOperation<T>/);
  ...
  assert.match(service, /code=STALE_RUN_EVENT/);
